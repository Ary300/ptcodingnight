...
function borderFor(invalid: boolean): string {
  ...
  return invalid ? "border-panther" : "border-ink/25";
}
...
export function Field({ label, hint, error, required = false, children }: FieldShellProps) {
  ...
  const describedBy =
    [hint !== undefined ? hintId : null, invalid ? errorId : null].filter((v) => v !== null).join(" ") ||
    undefined;
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {hint !== undefined && (
        <p id={hintId} className="opacity-70" style={{ fontSize: "var(--text-xs)" }}>
          {hint}
        </p>
      )}
      {children({ controlId, describedBy, invalid })}
      {invalid && (
        <p id={errorId} role="alert" className="font-semibold text-panther" style={{ fontSize: "var(--text-xs)" }}>
          {error}
        </p>
      )}
    </div>
